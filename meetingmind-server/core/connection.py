...
class ConnectionHandler:
    ...
    def language_of(self, text):
        ...
        for char in text:
            # 跳过非汉字字符（如标点、字母、数字等）
            if not '\u4e00' <= char <= '\u9fff' and not '\u3040' <= char <= '\u30ff':
                continue
    ...
    def speak_and_play(self, text, text_index=0):
        ...
        # chinese_text =self.llm.response_no_stream(
        #         system_prompt="如果是汉语，请你不做任何增减，直接输出用户的输入。否则请你翻译成汉语，直接给出翻译结果。", user_prompt=text
        #     )
        # japanese_text = self.llm.response_no_stream(
        #         system_prompt="如果是日语，请你不做任何增减，直接输出用户的输入。否则请你翻译成日语，直接给出翻译结果。", user_prompt=text
        #     )
        # print("origin_text", text)
