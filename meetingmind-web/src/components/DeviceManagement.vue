<template>
  <div style="padding: 5px 10px 15px;">
    ...
    <div class="content-panel">
      <div v-if="paginatedDeviceList.length === 0" class="no-data-container">
        <el-empty description="暂未绑定设备" />
      </div>
      <div v-else class="device-card-list">
        <el-checkbox-group v-model="selectedDevices" @change="handleSelectionChange">
            ...
            <el-card 
              ...
              <div class="card-content">
                ...
                <div class="card-header">
                  <el-checkbox :label="device.device_id"></el-checkbox>
                ...
                <div class="device-info">
                  ...
                  <div class="info-row">
                    ...
                    <span class="value">{{ device.bindTime }}</span>
                  </div>
                  <div class="info-row">
                    <span class="label">最近对话:</span>
                    <span class="value">{{ device.lastConversation }}</span>
                  </div>
                  <div class="info-row">
                    <span class="label">备注:</span>
                    <span class="value">
                      <el-input v-if="device.isEdit" v-model="device.remark" size="mini" :ref="`remarkInput-${index}`"
                        @blur="stopEditRemark(index)"></el-input>
                      <span v-else>
                        <i v-if="!device.remark" class="el-icon-edit" @click="startEditRemark(index, device)"></i>
                        <span v-else @click="startEditRemark(index, device)">
                          {{ device.remark }}
                        </span>
                      </span>
                    </span>
                ...
                <div class="card-footer">
                  ...
                  <div class="footer-right">
                    <el-switch v-model="device.otaSwitch" size="mini" active-color="#13ce66" inactive-color="#ff4949"
                  @change="handleOtaSwitchChange(device)"></el-switch>
                  </div>
                </div>
              </div>
            </el-card>
    ...
    <div v-if="paginatedDeviceList.length > 0" class="table_bottom">
      <div class="custom-pagination">
        <el-select v-if="isSuperAdmin" v-model="pageSize" @change="handlePageSizeChange" class="page-size-select">
            <el-option
        ...
        <button v-if="isSuperAdmin" class="pagination-btn" :disabled="currentPage === 1" @click="goFirst">首页</button>
        ...
        <span v-if="isSuperAdmin" class="total-text">共{{ deviceList.length }}条记录</span>
      </div>
    </div>

    <AddDeviceDialog :visible.sync="addDeviceDialogVisible" :agent-id="agentId"
      @refresh="fetchBindDevices(agentId)" />
...
export default {
  ...
  mounted() {
    if (this.agentId) {
      this.fetchBindDevices(this.agentId);
    }
  },
  methods: {
